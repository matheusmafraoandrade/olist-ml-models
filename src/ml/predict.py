...
# MAGIC %pip install feature-engine scikit-plot mlflow lightgbm
...
if not table_exists('silver.analytics', 'olist_models'):
    ...
    spark.sql(f"DELETE FROM {table} WHERE dtScore = {df_extract['dtReference'][0]} AND descModel = 'Churn Vendedor'")
    (df_spark.coalesce(1) 
            .write
            .format('delta')
            .mode('append')
...
# MAGIC %sql
# MAGIC 
# MAGIC SELECT * FROM silver.analytics.olist_models
